...
contract BettingMarket is Ownable, ReentrancyGuard, Pausable {
    ...
    function claimWinnings(uint256 _eventId) public nonReentrant {
        ...
        if (eventToClaim.winningOutcome && eventToClaim.betsA[msg.sender] > 0) {
            winnings = eventToClaim.betsA[msg.sender].mul(eventToClaim.totalBetsA.add(eventToClaim.totalBetsB)).div(eventToClaim.totalBetsA);
        } else if (!eventToClaim.winningOutcome && eventToClaim.betsB[msg.sender] > 0) {
            winnings = eventToClaim.betsB[msg.sender].mul(eventToClaim.totalBetsA.add(eventToClaim.totalBetsB)).div(eventToClaim.totalBetsB);
        }
    ...
    function getWinningOutcome(uint256 _eventId) public view returns (string memory) {
        Event storage eventToCheck = events[_eventId];
        require(eventToCheck.resolved, "Event not yet resolved");
        return eventToCheck.winningOutcome ? "A" : "B";
    }
